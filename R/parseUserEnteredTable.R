...
isNumericWithPercent <- function(t)
{
    v <- as.vector(t)
...
parseAsVectorOrMatrix <- function(m, warn)
{
    ...
    first.entry.chars <- !isNumericWithPercent(m[1, 1])
    idx <- if (first.entry.chars) -1
           else seq_len(n.row)
    row.names.given <- !isNumericWithPercent(m[idx, 1])
    idx <- if (first.entry.chars) -1
           else seq_len(n.col)
    col.names.given <- !isNumericWithPercent(m[1, idx])

    if ((row.names.given && col.names.given) ||
        (row.names.given && !first.entry.chars) ||
        (col.names.given && !first.entry.chars))
    {
        out <- matrix(asNumericWithPercent(m[2:n.row, 2:n.col]), nrow = n.row - 1)
    ...
    {  # somewhat ambiguous case, named 1,1 entry, but otherwise all numbers in first row
        out <- matrix(asNumericWithPercent(m[, 2:n.col]), nrow = n.row, ncol = n.col - 1)
        rownames(out) <- m[, 1]
        out <- drop(out)
    }else if (col.names.given)
    {  # somewhat ambiguous case, named 1,1 entry, but otherwise all numbers in first column
        out <- matrix(asNumericWithPercent(m[2:n.row, ]), nrow = n.row - 1, ncol = n.col)
        colnames(out) <- m[1, ]
        out <- drop(out)
    }
    else
        out <- matrix(asNumericWithPercent(m), nrow = n.row, ncol = n.col)
