...
#' @return character.
#' @importFrom flipU AllVariablesNames StopForUserError
...
GetTidyTwoDimensionalArray <- function(x, row.names.to.remove = NULL, column.names.to.remove = NULL)
{
    ...
    if (length(dim.x) != 2)
    {
        ...
        {
            StopForUserError("This analysis requires a two-dimensional table (i.e., a table with one set of row headings, one set of columns headings, and one statistic in each cell.")
        }
    }
