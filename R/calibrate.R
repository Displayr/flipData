                      ...
                      input.weight = NULL)
{
    ...
    if ((is.null(categorical.variables) || length(categorical.variables) == 0) && (is.null(numeric.variables) || length(numeric.variables) == 0)) {
        StopUserError("Nothing to do! At least one categorical OR numeric variable required.")
    }
    ...
    if (raking & min(sapply(targets, min)) == 0)
        StopUserError("One of your targets is set to 0. To achieve this, remove this category from the target and instead apply a filter to the weight to remove the category (or, in the R CODE set always.calibrate = TRUE)")
...
convertToDataFrame <- function(x)
{
    ...
    if (any(var.missing))
    {
        ...
        StopUserError("One or more of the adjustment variables contains missing values; you cannot have missing values in adjustment variables: ", var.missing)
    }
    x
}
...
categoricalTargets <- function(adjustment.variables, categorical.targets, subset)
{
    ...
    if (n.categorical != length(categorical.targets)) {
        StopUserError("The number of categorical adjustment variables needs to be the same as the number of sets of targets (it isn't)")
    }
    for (i in 1 : n.categorical)
    {
        ...
        if(any(idx <- is.na(targets[[i]])))
        {
            ...
            StopUserError("Invalid target values for ", varname, ": ", bad.inputs, ".")
        }
        ...
        if (any(missing.targets))
        {
            ...
            if (nchar(missing.cats) > 0)
                StopUserError("No targets have been provided for ", varname, ": ", missing.cats)
        }
        ...
        if (any(excess.targets))
        {
            ...
            if (nchar(excess.cats) > 0)
                StopUserError("Target category that does not appear in variable ", varname, ": ",
        ...
        if ((sm = Sum(targets[[i]], remove.missing = FALSE)) != 1)
        {
            if (round(sm, 6) == 1)# Forcing to add up to 1 if difference likely due to rounding errors.
                ...
                StopUserError("Targets for ", varname, " add up to ", sm, "; they need to add up to exactly 1.")
        }
...
numericTargets <- function(targets, adjustment.variables, numeric.targets, subset)
{
    ...
    {
        StopUserError("The number of numeric adjustment variables needs to be the same as the number of sets of targets (it isn't)")
    }
    ...
    for (i in 1:n.numeric)
    {
        ...
        if (is.na(tgt))
            StopUserError("The target for ", varname, " is invalid.")
        if (!is.numeric(numvar))
            StopUserError(varname, " is not a numeric variable; numeric adjustment variables must be numeric.")
        if (max(numvar) < tgt)
            StopUserError("The target for ", varname, " of ", tgt, " is higher than the maximum value of the variable", (if(is.null(subset)) "" else " (after applying filter/subset)"))
        if (min(numvar) > tgt)
            StopUserError("The target for ", varname, " of ", tgt, " is lower than the minimum value of the variable", (if(is.null(subset)) "" else " (after applying filter/subset)"))
    }
    targets
}
...
checkSolverStatus <- function(solve.output)
{
    if (is.list(solve.output) && is.character(solve.output[["status"]]))
    {
        ...
        if (status == "solver_error" || status == "infeasible")
            StopUserError("Calibration could not be performed for the given input data. ",
